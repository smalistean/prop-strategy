...
public final class BacktestApplication {
    ...
    public static void main(String[] args) {
        ...
        BacktestBarAssembler.AssembledDataset dataset = BacktestBarAssembler.assemble(
                ...
                marketSymbol, strategy, loaded.engine().execution().makerEnabled(),
                loaded.dataset().startInclusive(), loaded.dataset().endExclusive());
        List<BacktestEngine.BacktestBar> bars = dataset.bars();
        List<FundingRate> funding = dataset.funding();
        List<Kline> minuteCandles = dataset.minuteCandles();
        ...
        System.out.printf("Backtest: dataset=%s [%s, %s), strategy=%s, symbol=%s, interval=%s, "
                ...
                loaded.interval(), dataset.loadedCandles(), bars.size());
